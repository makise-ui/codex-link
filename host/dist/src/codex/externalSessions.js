...
export async function listExternalSessions(options = {}) {
...
export async function findExternalSession(externalSessionId, options = {}) {
...
export async function readExternalSessionHistory(session, limit = 160) {
    ...
    for (const line of raw.split(/\r?\n/).filter(Boolean)) {
        ...
        if (event.type === "event_msg" && payload.type === "user_message") {
            ...
            if (text)
                messages.push(storedMessage({ role: "user", text, timestamp }));
            continue;
        }
        ...
        if (payload.type === "message") {
            ...
            if (text)
                messages.push(storedMessage({ role, text, timestamp, title: role === "assistant" ? "Response" : undefined }));
            continue;
        }
...
async function readExternalSession(filePath) {
...
function titleFromLines(lines) {
    for (const line of lines) {
        ...
        if (payload.type === "message" && payload.role === "user" && Array.isArray(payload.content)) {
            ...
            if (text.trim() && !isEnvironmentContext(text))
                return compactTitle(text);
        }
    }
    return undefined;
