...
export class FileTransferManager {
    ...
    async resolveWorkspaceRoot(workspaceRoot) {
        const resolved = await realpath(path.resolve(workspaceRoot));
        const allowed = await Promise.all(this.workspaces.map(async (workspace) => realpath(path.resolve(workspace.path)).catch(() => path.resolve(workspace.path))));
...
export function mimeTypeFor(filePath) {
    ...
    if (ext === ".gif")
        return "image/gif";
    if (ext === ".dart" || ext === ".ts" || ext === ".js" || ext === ".tsx" || ext === ".jsx")
        return "text/plain";
    return undefined;
}
