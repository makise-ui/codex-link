...
describe("CliCodexSession", () => {
    ...
    it("streams tool lifecycle events as visible started, delta, and completed messages", async () => {
        ...
        const thinkingStarted = events.filter((event) => event.type === "message.started" && event.kind === "thinking");
...
async function waitForCompletion(events, timeoutMs = 5_000) {
...
async function waitForOutput(events, text, timeoutMs = 1_000) {
    const startedAt = Date.now();
