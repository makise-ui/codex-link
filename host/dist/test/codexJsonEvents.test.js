...
describe("Codex JSONL mapping", () => {
    ...
    it("maps raw Codex read-command completions with aggregated output", () => {
        expect(mapCodexJsonEvent(parseCodexJsonLine('{"type":"item.completed","item":{"id":"cmd-1","type":"command_execution","command":"/usr/bin/zsh -lc \\"cat notes.txt\\"","aggregated_output":"Line 1\\nLine 2\\n","exit_code":0,"status":"completed"}}'))).toEqual({
            kind: "message",
    ...
    it("maps raw Codex file changes to visible editing activity", () => {
        expect(mapCodexJsonEvent(parseCodexJsonLine('{"type":"item.started","item":{"id":"edit-1","type":"file_change","changes":[{"path":"/repo/notes.txt","kind":"add"},{"path":"/repo/lib/app.dart","kind":"update"}],"status":"in_progress"}}'))).toEqual({
            kind: "message_started",
            messageKind: "executing",
            title: "Editing files",
