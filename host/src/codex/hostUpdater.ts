...
import type { HostUpdateProgressMessage, HostUpdateResultMessage, HostUpdateStatusMessage } from "../protocol/messages.js";

const execFileAsync = promisify(execFile);
const DEFAULT_PACKAGE_NAME = "codex-link-host";
const MAX_OUTPUT_CHARS = 32 * 1024;

export type HostUpdateProgressCallback = (message: HostUpdateProgressMessage) => void;

export class HostPackageUpdater {
  private running = false;

  constructor(private readonly packageName = DEFAULT_PACKAGE_NAME) {}

  get isRunning(): boolean {
    return this.running;
  }

  async check(): Promise<HostUpdateStatusMessage> {
  ...
  async run(onProgress: HostUpdateProgressCallback): Promise<HostUpdateResultMessage> {
    if (this.running) {
...
async function readCurrentPackageVersion(packageName: string): Promise<string> {
  const start = path.dirname(fileURLToPath(import.meta.url));
  const packageJsonPath = await findPackageJson(start, packageName);
  const content = await readFile(packageJsonPath, "utf8");
  const record = JSON.parse(content) as { version?: unknown };
  return typeof record.version === "string" && record.version.trim().length > 0
    ? record.version.trim()
    : "0.0.0";
}

async function findPackageJson(startDir: string, packageName: string): Promise<string> {
  let current = startDir;
  for (let index = 0; index < 8; index += 1) {
    const candidate = path.join(current, "package.json");
    try {
      const content = await readFile(candidate, "utf8");
      const record = JSON.parse(content) as { name?: unknown };
...
async function readLatestPackageVersion(packageName: string): Promise<string> {
...
function runNpmInstallLatest(packageName: string, onProgress: HostUpdateProgressCallback): Promise<{ exitCode: number; stdout: string; stderr: string }> {
...
function emitProgressLines(packageName: string, text: string, onProgress: HostUpdateProgressCallback): void {
